...
public class RangeSeekBar<T extends Number> extends ImageView {
    ...
    private static final int LINE_HEIGHT_IN_DP = 122;
     ...
     * custom
     */
    private boolean mIsChangeSeeker = false;
    ...
    private void init(Context context, AttributeSet attrs) {
        /** custom **/
        int thumbLeft = R.drawable.btn_left_cut;
        int thumbRight = R.drawable.btn_right_cut;

        float barHeight;
        int thumbNormal = R.drawable.btn_left_cut;
        int thumbPressed = R.drawable.btn_right_cut;
        int thumbDisabled = R.drawable.btn_right_cut;
...
//            barHeight = PixelUtil.dpToPx(context, LINE_HEIGHT_IN_DP);
            barHeight = getResources().getDimensionPixelOffset(R.dimen.dp_35);
            mActiveColor = ACTIVE_COLOR;
            mDefaultColor = Color.parseColor("#69b06c");
            mAlwaysActive = false;
...
//                barHeight = a.getDimensionPixelSize(R.styleable.RangeSeekBar_barHeight, LINE_HEIGHT_IN_DP);
                ...
                mDefaultColor = a.getColor(R.styleable.RangeSeekBar_defaultColor, Color.parseColor("#69b06c"));
                mAlwaysActive = a.getBoolean(R.styleable.RangeSeekBar_alwaysActive, false);

                Drawable leftDrawable = a.getDrawable(R.styleable.RangeSeekBar_thumbNormal);
                if (leftDrawable != null) {
                    thumbLeftImage = BitmapUtil.drawableToBitmap(leftDrawable);
                }
                Drawable rightDrawable = a.getDrawable(R.styleable.RangeSeekBar_thumbNormal);
                if (leftDrawable != null) {
                    thumbRightImage = BitmapUtil.drawableToBitmap(rightDrawable);
                }
        ...
        if (thumbLeftImage == null) {
            thumbLeftImage = BitmapFactory.decodeResource(getResources(), thumbLeft);
        }
        if (thumbRightImage == null) {
            thumbRightImage = BitmapFactory.decodeResource(getResources(), thumbRight);
        }
...
//        mThumbHalfHeight = thumbImage.getHeight();
        ...
        /** custom **/
//        mRect = new RectF(padding,
//                10 + mDistanceToTop + mThumbHalfHeight,
//                getWidth() - padding,
//                20 + mThumbHalfHeight + mThumbHalfHeight + mThumbHalfHeight);
    ...
    int playIndicator = 10;

    public void setPlayIndicator(int playIndicator) {
...
//        paint.setColor(colorToUseForButtonsAndHighlightedLine);
        ...
        drawLeftThumb(normalizedToScreen(normalizedMinValue), Thumb.MIN.equals(pressedThumb), canvas, selectedValuesAreDefault);
...
//                selectedValuesAreDefault);
        drawRightThumb(normalizedToScreen(normalizedMaxValue), Thumb.MAX.equals(pressedThumb), canvas, selectedValuesAreDefault);

        if (!mIsChangeSeeker) {
//            JYLog.D("fdjakkljdklfjadkfjal::" + playIndicator, new Throwable());
            drawPlayIndicator(normalizedToScreen(normalizedMaxValue), playIndicator, canvas);
        }
    ...
    private void drawPlayIndicator(float screenCoord, int x, Canvas canvas) {

        Paint paint = new Paint(Paint.ANTI_ALIAS_FLAG);
        paint.setColor(Color.parseColor("#ffffff"));
        paint.setStrokeWidth(getResources().getDimension(R.dimen.dp_2));
        paint.setStyle(Style.STROKE);
        canvas.drawLine(x, 0, x, canvas.getHeight() + 10, paint);
