 ...
 */
public class InniBitmapThread2 extends Thread {

    private Context mContext;
    private Bitmap bitmap;
    private Handler handler;

    public InniBitmapThread2(Context mContext, Bitmap bitmap, Handler handler) {
        this.mContext = mContext;
        this.bitmap = bitmap;
        this.handler = handler;
    }

    public InniBitmapThread2(Context mContext, String absolutePath, int width, Handler handler) {
        this.mContext = mContext;
        this.handler = handler;
    ...
    public void run() {
        ...
        Bitmap inniFilterBitmap = BitmapFactory.decodeResource(mContext.getResources(), R.drawable.f_b2d0af, options);

        GPUImageAlphaBlendFilter alphaBlendFilter = new GPUImageAlphaBlendFilter();
        alphaBlendFilter.setBitmap(inniFilterBitmap);
        alphaBlendFilter.setMix(0.29f);
